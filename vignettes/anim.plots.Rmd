...
anim.plot(1860:1959, discoveries, times=1:100, xlim=xlim, col="red", xlab="Year",
...
These functions do not respect changes in animation speed. For example, here's
a rocket boosting off:
...
```{r, eval=FALSE}
anim.save(replay(tmp, smooth.fps=10), type="Video", filename="rocket.mp4")
```

**Note**: `smooth.fps` only works in version 0.2 and above (available from GitHub as of November 2014).

The `smooth.fps` argument creates a "smoothed" version of the animation, which
...
The code lives at http://github.com/hughjonesd/anim.plots.
